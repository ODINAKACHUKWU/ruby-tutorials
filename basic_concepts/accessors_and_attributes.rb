...
class Ticket
  ...
  def price
    puts "The ticket for #{event} costs $#{format('%.2f', @price)}."
  end

  def discount(percent)
    @price = @price * (100 - percent) / 100.0
    puts "The ticket for #{event} has been discounted $#{'%.2f' % percent} to $#{format('%.2f', @price)}."
  end
end
